...
const makeEntry = (id: number, path: string, status: FileStatus = 'pending') => ({
  id,
  path,
  relative_path: path.replace('/data/', ''),
  size_bytes: 1024,
  sha256: 'abc',
  mime_type: null,
  status,
...
describe('FileBrowser', () => {
  ...
  it('debounces search and calls getFileEntries', async () => {
    ...
    await waitFor(
      () =>
        expect(getFileEntries).toHaveBeenCalledWith(expect.objectContaining({ search: 'photo' })),
      { timeout: 1000 }
    )
  })
  ...
  it('filters by status when select changes', async () => {
    ...
    await import('@testing-library/user-event').then(({ default: u }) =>
      u.selectOptions(select, 'keep')
    )
    await waitFor(() =>
      expect(getFileEntries).toHaveBeenCalledWith(expect.objectContaining({ status: 'keep' }))
    )
  })
